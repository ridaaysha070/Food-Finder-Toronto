...
class Restaurant:
    ...
    def __init__(self, name: str, cuisine: str, price_range: tuple[int, int], address: str,
                 star_rating: float, phone: str, coordinates: tuple[float, float], distance: str):
...
def get_restaurant_info(lst: list[Restaurant], user: User) -> str:
    """Display information about this restaurant

    Preconditions:
    - len(lst) == len(data)
    """
    rest = input('Enter the name of the restaurant you want more information about')
    ...
    for i in smaller_list:
        if i.name == rest:
            ...
            return f'This restaurant is located {i.distance[1]} km away at {clean_ad}'
