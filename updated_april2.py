...
def get_user_info(user: User) -> None:
    ...
    """
    location = input('Enter the address you would like to find restaurants near (e.g.,  Xyz St, Toronto, Ontario):')
    answers = get_user_input(RESTAURANT_QUESTIONS)
    user.questions = answers
    user.questions[1] = answers[1].capitalize()
    user.questions[2] = answers[2].capitalize()
    if location:
...
def run_restaurant_finder_test(user: User) -> None:  # User object must be created first
    """
    ...
    if not rests:
        ...
        print(f'Restaurant: {rests[0][0].name}')
    else:
        for restaurant in rests:
            user.recommendations.append(restaurant[0])
            print(f'Restaurant: {restaurant[0].name}\n')
...
def run_restaurant_finder() -> None:  # User object must be created first
    """
    Create a user object and find restaurants for that user based on their requirements.
    """
    user = User()
...
def get_restaurant_info(user: User) -> None:
    ...
    for i in user.recommendations:
        if i.name == rest and i.distance[0] == user.questions[2]:
            ...
            if review_info == 'yes':
                print(f"This restaurant's Yelp rating is {i.star_rating}.")
            matches.append(i)

    if len(matches) > 1 and all(x.contact[1] == y.contact[1] for x in matches for y in matches):
